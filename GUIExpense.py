...
header = ['ລະຫັດລາຍການ','ວັນ-ເວລາ','ລາຍການ','ຄ່າໃຊ້ຈ່າຍ','ຈຳນວນ', 'ລວມ']
...
headerwidth = [120,150,120,80,60,80]
...
def UpdateCVS():
	with open('savedata.csv','w',newline='',encoding='utf-8') as f:
		fw = csv.writer(f)
		# ກຽມຂໍ້ມູນຈາກ alltransaction ໃຫ້ກາຍເປັນ list
		data = list(alltransaction.values())
		fw.writerows(data) # multiple line from nested list [[],[],[]]
		print('Table was updated')

def DeleteRecord(event=None):
	check = messagebox.askyesno('Confirm?','ເຈົ້າຕ້ອງການລົບຂໍ້ມູນ ຫຼື ບໍ?')
	print('YES/NO:',check)
